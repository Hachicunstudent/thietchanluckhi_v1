/* global cv */
// detectArea.js
export const detectAreaImg = (croppedImageData) => {
    return new Promise((resolve, reject) => {
        if (!window.cv) {
            reject('OpenCV không sẵn sàng.');
            return;
        }
        ...
        image.onload = () => {
            try {
                ...
                // Giải phóng bộ nhớ
                src.delete();
                resolve(detectAreaImageData); // Trả về hình ảnh đã xử lý
            } catch (error) {
                reject('Lỗi khi xử lý hình ảnh trong OpenCV: ' + error);
            }
        };
        image.onerror = () => {
            reject('Lỗi khi tải hình ảnh.');
        };
    });
};
